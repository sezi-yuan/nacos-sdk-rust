...
async fn gen_client_from_env() -> NamingClient<HttpNamingRemote> {
    use std::env::var;
    let namespace = var("NACOS.NAMESPACE").unwrap_or(constants::DEFAULT_NAMSPACE.to_owned());
    let group = var("NACOS.GROUP").unwrap_or(constants::DEFAULT_GROUP.to_owned());
    let cluster = var("NACOS.CLUSTER").unwrap_or(constants::DEFAULT_CLUSTER.to_owned());
    let server_schema = var("NACOS.SERVER_SCHEMA").unwrap_or(constants::DEFAULT_SERVER_SCHEMA.to_owned());
    let server_context = var("NACOS.SERVER_CONTEXT").unwrap_or(constants::DEFAULT_SERVER_CONTEXT.to_owned());
     let servers = var("NACOS.SERVERS").expect("env NACOS.SERVERS is empty")
        .split(",")
        .map(|server| ServerConfig::new(server_schema.clone(), server.to_string(), server_context.clone()))
        .collect::<Vec<_>>();
    let failover_dir =var("NACOS.NAMING.FAILOVER").unwrap_or_else(|_|{
    ...
    let config = NamingConfig {
        ...
        load_at_start: parse_bool_env("NACOS.NAMING.LOAD_AT_START", false),
        update_when_empty: parse_bool_env("NACOS.NAMING.LOAD_AT_START", false),
        user_name: var("NACOS.USERNAME").ok(),
        password: var("NACOS.PASSWORD").ok(),
    };
